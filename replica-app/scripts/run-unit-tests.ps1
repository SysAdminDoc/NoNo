...
Push-Location $root
try {
    # Force execution: an UP-TO-DATE task produces no evidence for this run, and evidence
    # is the whole point of invoking the suite through this script.
    & (Join-Path $root 'gradlew.bat') --no-daemon testDebugUnitTest --rerun-tasks
...
if ($exitCode -eq 0 -and $summary.tests -eq 0) {
    throw 'Gradle reported success but produced no test results. Refusing to record evidence for a suite that did not run.'
}
