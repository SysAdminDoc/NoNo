...
Version 1.6.0 is a correctness pass: user regular expressions are bounded in both the text they
search and the time they may spend, history search no longer treats typed punctuation as SQL
wildcards, the system bars follow the app's theme, a failed export cannot destroy the file it was
overwriting, and several settings rows stopped claiming to do things this build does not do.
Version 1.5.1 fixed the action chooser losing a typed filter when the screen is recreated, and
recorded the first on-device check of the no-backup boundary. Version 1.5.0 added an Insights
...
```powershell
& "$env:LOCALAPPDATA\Android\Sdk\build-tools\37.0.0\apksigner.bat" verify --print-certs NoNo-v1.6.0-reconstruction.apk
```
...
unavailable and is never matchable as real text. Preferences and history live under the no-backup boundary, and that boundary has been checked against the platform rather than assumed. On 2026-08-31, on an API 36 device, a real `bmgr` backup and restore cycle ran against a build holding captured history. A control file placed in the ordinary files directory came back afterwards with its contents intact, so the transfer itself worked. Nothing the app had stored came back. Two different mechanisms produce that result, and they are worth separating. The history database, the store holding rules, and the pseudonym key sit in `noBackupFilesDir`, which Android excludes on its own, so the exclusion entry in `res/xml` naming the standard database directory is inert: Room opens the file by absolute path and nothing is ever written to the location that entry describes. The listener's own scratch preferences are different. They are ordinary shared preferences in the backed-up tree, and the `res/xml` exclusions are the only thing keeping them out of a transfer. Listener diagnostics restore
