...
fun oemListenerChecklist(manufacturer: String, sdkInt: Int): List<String> {
    ...
    steps += "Allow unrestricted battery use for NoNo in Android's battery settings."
    when (manufacturer.lowercase()) {
        "samsung" -> {
            steps += "Open Device care, then Battery, and add NoNo to Apps that won't be put to sleep."
            steps += "Turn off Put unused apps to sleep and Auto disable unused apps."
        }
        "xiaomi", "redmi", "poco" -> {
            steps += "Turn on Autostart for NoNo in Security, then Permissions."
            steps += "Set Battery saver for NoNo to No restrictions."
            steps += "Lock NoNo in Recents so the memory cleaner leaves it alone."
        }
        "huawei", "honor" -> {
            steps += "Set NoNo to Manage manually in Battery, then App launch."
            steps += "Enable Auto-launch, Secondary launch and Run in background."
        }
        "oneplus", "oppo", "realme" -> {
            steps += "Allow Auto-launch for NoNo."
            steps += "Set background power use to Allow, and turn off Deep optimisation and Sleep standby optimisation."
        }
        "vivo", "iqoo" -> {
            steps += "Allow Auto-start and High background power consumption for NoNo."
        }
        else -> {
            steps += "Look for an autostart, protected apps, or background restriction list in your battery settings and allow NoNo there."
        }
    }
    steps += "Lock NoNo in the Recents view if your launcher offers it."
    if (sdkInt >= 33) {
